    ...
    if (num !== -1) {
      if (LocalStorage.user) {
        ...
        if (LocalStorage.user.hasOwnProperty('professional_card')) {
          ...
          this.setState({
            visits: <Text>{`${localization.numVisits} ${num}`}</Text>,
          });
        } else {
  ...
  getNationality() {
    ...
    if (LocalStorage.user) {
      ...
      if (language === 'en') {
        ...
        nationality =
          LocalStorage.user.nationality !== null
            ? LocalStorage.user.nationality.name_ar
            : '';
      }
      return nationality;
    }
        ...
        <Content contentContainerStyle={{ flex: 1, justifyContent: 'center' }}>
          <View style={styles.container}>
            <View style={styles.topContainer}>
              {LocalStorage.user ? (
                <Avatar page="drawer" image={LocalStorage.user.image} />
              ) : (
                <Thumbnail
                  circle
                  large
              ...
              {LocalStorage.user ? <Text>{LocalStorage.user.name}</Text> : null}

              {LocalStorage.user ? <Text>{this.getNationality()}</Text> : null}
          ...
          <View style={styles.buttonsContainer}>
            ...
            >
              ...
              {LocalStorage.user ? (
                LocalStorage.user.hasOwnProperty('professional_card') ? (
                  <Button
                    full
                    onPressIn={() =>
              ...
              {LocalStorage.user ? (
                !this.props.loadingLogout ? (
                  <Button
                    full
                    onPressIn={() => {
                      this.props.logout({ token: LocalStorage.user.token });
